...
def fit_one_epoch(model_train, model, ema, yolo_loss, loss_history, eval_callback, optimizer, epoch, epoch_step,
    ...
    for iteration, batch in enumerate(gen):
        ...
        images, bboxes, clearimgs = batch[0], batch[1], batch[2]
        with torch.no_grad():
            if cuda:
                images = images.cuda(local_rank)
                bboxes = bboxes.cuda(local_rank)
                clearimgs = clearimgs.cuda(local_rank)
                hazy_and_clear = torch.cat([images, clearimgs], dim=0).cuda()

        optimizer.zero_grad()
        if not fp16:
            outputs = model_train(hazy_and_clear)
            loss_value = yolo_loss(outputs[0:-1], bboxes)
            loss_dehazy = criterion(outputs[-1], clearimgs)
            loss_value = 0.55 * loss_value + 0.45 * loss_dehazy
            loss_value.backward()
